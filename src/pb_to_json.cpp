...
bool PbToJsonConverter::Convert(const google::protobuf::Message& message, Handler& handler) {
    ...
    for (int i = 0; i < field_count; ++i) {
        ...
        if (_option.enable_protobuf_map && protobuf_json::IsProtobufMap(field)) {
    ...
    for (size_t i = 0; i < map_fields.size(); ++i) {
        ...
        const google::protobuf::FieldDescriptor* key_desc =
                map_desc->message_type()->field(protobuf_json::KEY_INDEX);
        const google::protobuf::FieldDescriptor* value_desc =
                map_desc->message_type()->field(protobuf_json::VALUE_INDEX);
...
#define CASE_FIELD_TYPE(cpptype, method, valuetype, handle)             \
    ...
    CASE_FIELD_TYPE(INT32,  Int32,  int,          Int);
    CASE_FIELD_TYPE(UINT32, UInt32, unsigned int, Uint);
    CASE_FIELD_TYPE(INT64,  Int64,  int64_t,      Int64);
    CASE_FIELD_TYPE(UINT64, UInt64, uint64_t,     Uint64);
    ...
    case google::protobuf::FieldDescriptor::CPPTYPE_ENUM: {
        if (field->is_repeated()) {
            ...
            if (_option.enum_option == OUTPUT_ENUM_BY_NAME) {
                ...
                for (int index = 0; index < field_size; ++index) { 
                    handler.Int(reflection->GetRepeatedEnum(
            ...
            if (_option.enum_option == OUTPUT_ENUM_BY_NAME) {
                ...
                handler.Int(reflection->GetEnum(message, field)->number());
            }
        }
        break;
    }
...
bool ProtoMessageToJson(const google::protobuf::Message& message,
    ...
    if (protobuf_json::ProtoMessageToJsonStream(message, options, buffer, error)) {
...
bool ProtoMessageToJson(const google::protobuf::Message& message,
    ...
    protobuf_json::ZeroCopyStreamWriter wrapper(stream);
    return protobuf_json::ProtoMessageToJsonStream(message, options, wrapper, error);
}
